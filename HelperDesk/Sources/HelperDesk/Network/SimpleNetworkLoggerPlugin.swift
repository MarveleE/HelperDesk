//
//  MoyaNetwork+Plugins.swift
//  PMM-iOS
//
//  Created by keyu on 2023/7/30.
//
...
extension SimpleNetworkLoggerPlugin: PluginType {
    ...
    public func didReceive(_ result: Result<Moya.Response, MoyaError>, target: TargetType) {
        ...
        case .success(let success):
            loggings.append("URL: \(success.request?.url?.absoluteString ?? target.baseURL.absoluteString + target.path)")
        ...
        case .failure(let failure):
            loggings.append("URL: \(failure.response?.request?.url?.absoluteString ?? target.baseURL.absoluteString + target.path)")
            loggings.append("Method: \(target.method.rawValue)")
